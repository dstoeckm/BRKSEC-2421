...
// generate .sh script to demo dlp policy
resource "local_file" "dlp10" {
  content  = <<-DOC
    curl -kv curl -kv "https://www.example.com/conn.php?data=callback="
    DOC
  filename = "roles/opencart-aws/files/dlp10.sh"
}
// generate .sh script to demo dlp policy
resource "local_file" "dlp11" {
  content  = <<-DOC
    curl -kv curl -kv "https://www.example.com/conn.php?data=callback="
    DOC
  filename = "roles/opencart-aws/files/dlp11.sh"
}
